...
**仅按文件大小切分**（底层 lumberjack v2 只支持 size 触发，不支持按天/小时切分；如需按天切分请用 logrotate 等系统工具）：
- 当前文件达到 `MaxSize` MB 时触发切分

`MaxAge`（天数）控制**旧文件的保留清理**，不触发切分；`MaxBackups` 控制旧文件数量上限。

切分后：
1. 当前文件重命名为带时间戳的备份（lumberjack 实际格式含纳秒+时区，形如 `app-2026-06-23T10-00-00.000000000-07-00.log`）
2. 创建新 `app.log` 继续写入
