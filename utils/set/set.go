...
//
// 语义：Set 基于内部 map 引用，采用 pointer receiver 共享同一底层 map。
// 因此 s2 := s1 后 s2 与 s1 指向同一集合（与 *sync.Map、*list.List 等引用语义一致）。
// 如需独立副本请显式 Clone()。
//
// 并发安全：
//   - Set 本身非线程安全；并发使用请外部加锁（如 sync.Mutex）或用 sync.Map 替代
package set

import "sort"

// Set 是基于 map 的泛型集合（pointer receiver，引用语义）。
// 必须经 New / NewWithCapacity / FromSlice 构造，零值 Set 不可直接使用。
...
// Clone 返回 Set 的独立副本（深拷贝底层 map，修改副本不影响原集合）
